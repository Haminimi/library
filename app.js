			...
			deleteButton.addEventListener('click', function () {
				if (cards.childElementCount > 1) {
					...
					if (favoriteBooksList.textContent.includes(book.title)) {
						const paragraphs =
							favoriteBooksCard.querySelectorAll('p');
					...
					if (favoriteBooksList.textContent.includes(book.title)) {
						const paragraphs =
							favoriteBooksList.querySelectorAll('p');
						paragraphs.forEach((para) => {
							if (
								book.author === '' &&
								para.textContent === `${book.title}`
					...
					emptyLibraryCardText.classList.add(
						'empty-library-card-text'
					);
					emptyLibraryCard.appendChild(emptyLibraryCardText);
					const emptyLibraryCardButtonDiv =
						document.createElement('div');
					emptyLibraryCardButtonDiv.classList.add(
						'empty-library-card-button-div'
					);
					const emptyLibraryCardButton =
						document.createElement('span');
					emptyLibraryCardButton.classList.add(
						'new-book-button',
						'material-symbols-outlined'
					);
					emptyLibraryCardButton.textContent = 'add';
					emptyLibraryCardButtonDiv.appendChild(
						emptyLibraryCardButton
					);
